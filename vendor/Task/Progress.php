...
class Task_Progress {
    ...
    protected function tryToResetWrongItems() {
        ...
        foreach ($wrongItems as $item) {
            ...
            Service()->logger->debug('task try to reset wrong items', $item);
        }
    }
    ...
    protected function runAllWaittingItems() {
        ...
        foreach ($waittingItems as $item) {
            ...
            if (empty($class) || !class_exists($class)) {
                Service()->logger->error('Error: task can not run illegal class', $item);
            ...
            if (!is_callable(array($class, 'fire'))) {
                Service()->logger->error('Error: task can not call fire()', $item);
