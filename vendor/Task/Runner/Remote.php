...
class Task_Runner_Remote extends Task_Runner {
    ...
    public function __construct(Task_MQ $mq, $step = 10, Task_Runner_Remote_Connector $contector = NULL) {
        $config = Service()->config->get('app.Task.runner.remote');

        if ($contector === NULL) {
            if (empty($config['host'])) {
                throw new Kernel_Exception_InternalServerError(T('task miss api host for'));
            }
    ...
    protected function youGo($service, $params) {
        ...
        if ($this->contector->getRet() == 404) {
            throw Kernel_Exception_InternalServerError('task request api time out',
        ...
        if (!$isOk) {
            Service()->logger->debug('task remote request not ok',
