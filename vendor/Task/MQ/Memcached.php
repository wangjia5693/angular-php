...
class Task_MQ_Memcached extends Task_MQ_KeyValue {
    ...
    public function __construct(Kernel_Cache $mcCache = NULL) {
        if ($mcCache === NULL) {
            $config = Service()->config->get('app.Task.mq.mc');
            ...
            $mcCache = extension_loaded('memcached') 
                ? new Kernel_Cache_Memcached($config)
                : new Kernel_Cache_Memcache($config);
        }
