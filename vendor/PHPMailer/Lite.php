...
class PHPMailer_Lite
{
    ...
    public function __construct($debug = FALSE) {
        ...
        $this->config = Service()->config->get('app.PHPMailer.email');
    }
    ...
    public function send($addresses, $title, $content, $isHtml = TRUE)
    {
        ...
        if (!$mail->send()) {
            if ($this->debug) {
                Service()->logger->debug('Fail to send email with error: ' . $mail->ErrorInfo);
            }
        ...
        if ($this->debug) {
            Service()->logger->debug('Succeed to send email', array('addresses' => $addresses, 'title' => $title));
        }
