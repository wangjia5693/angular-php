...
class Kernel_Cookie_Multi extends Kernel_Cookie {

	/**
	 * @param $config['crypt'] 加密的服务，如果未设置，默认取Service()->crypt，须实现PhalApi_Crypt接口
	 * @param $config['key'] $config['crypt']用的密钥，未设置时有一个md5串
	 */
    public function __construct($config = array()) {
        parent::__construct($config);

        $this->config['crypt'] = isset($config['crypt']) ? $config['crypt'] : Service()->crypt;

        if (isset($config['crypt']) && $config['crypt'] instanceof Kernel_Crypt) {
