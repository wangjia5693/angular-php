...
class Kernel_Request {
    ...
    public function getByRule($rule) {
        ...
        if (!isset($rule['name'])) {
            throw new Kernel_Exception_InternalServerError(T('miss name for rule'));
        }
        // 多态
        $rs = Kernel_Request_Var::format($rule['name'], $rule, $this->data);

        if ($rs === NULL && (isset($rule['require']) && $rule['require'])) {
            throw new Kernel_Exception_BadRequest(T('{name} require, but miss', array('name' => $rule['name'])));
        }
